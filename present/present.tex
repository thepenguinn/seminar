...
\def\title{TinyML in Agriculture Sector}
...
\begin{frame} {}
    \begin{tikzpicture} [
        ...
        \draw (current page.north west) ++(1.00cm, -1.75cm) coordinate (frametitle) ;
        ...
        (frametitle.south -| current page.west) ++(1.25, -0.5) coordinate (lineStart)
        (lineStart) ++(8, 0) coordinate (lineEnd)

        (lineStart) -- (lineEnd)

        ;

        \draw (current page.north west |- lineStart) ++(1.00cm, -0.75cm) coordinate (content) ;
    ...
    \begin{tikzpicture} [
        ...
        \node [anchor = north west]  (content) at (content) {
            \begin{minipage} {8.75cm}
                ...
                They \alert{Convert} / \alert{Extract} / \alert{Manipulate} Data in different ways.
...
\begin{frame} {}
    ...
    \begin{tikzpicture} [
        ...
        \draw (frametitle.south west) ++(0, -0.7cm) coordinate (content) ;
        \node [anchor = north west]  (content) at (content) {
        ...
        \node [anchor = north west]  (content) at (content) {
            \begin{minipage} {8.1cm}
                The \alert{coefficients} / \alert{weights} in our question matrix decides the output
                we get.
            \end{minipage}
        }
        ;
        ...
        \node [anchor = north east]  (content) at (content) {
            \begin{minipage} {0.5\textwidth}
                ...
                \begin{itemize}
                    ...
                    \item FPGAs
                    \item DSPs
                    \item Anything that is low powered and resource constrained.
                \end{itemize}
...
\begin{frame}
    ...
    \begin{tikzpicture} [
        ...
        \node [anchor = center] at (s2-center) {
            \begin{minipage} {\squareSide}
                \centering
                \large Improved \\ Network \\ Latency
            \end{minipage}
        }
        ;
...
\begin{frame} {}
    ...
    \begin{tikzpicture} [
        ...
        \tikzframetitleright{Improved} {Network Latency}
        ...
        node [anchor = east] {
            \begin{minipage} {0.425\textwidth}
                ...
                \begin{itemize}
                    ...
                    \item So literally \alert{Zero Network Latency}.

                    \item Can be deployed to \alert{Remote Locations},
    ...
    \begin{tikzpicture} [
        ...
        \node [anchor = north west]  (content) at (content) {
            \begin{minipage} {8.0cm}
                ...
                \alert{14 kilo-byte (KB)} neural network that ran on embedded
                DSPs having only \alert{tens of kilo-bytes of memory}!
        ...
        node [anchor = east] {
            \begin{minipage} {0.575\textwidth}
                ...
                \begin{itemize}
                    ...
                    \item Sometimes we need to \alert{cascade} different models having different fidelity.
...
\begin{frame} {}
    ...
    \begin{tikzpicture} [
        ...
        node [anchor = west] {
            \begin{minipage} {0.75\textwidth}
                \begin{itemize}
                    \item Developed by \alert{Google} to build and deploy ML Models on Embedded Systems.
    ...
    \begin{tikzpicture} [
        ...
        node [anchor = north west] {
            \begin{minipage} {9.85cm}
                \begin{itemize}
                    ...
                    \item Held a comparative study on \alert{TinyML} based system vs \alert{Cloud} based system.
        ...
        node [
            ...
            \begin{minipage} {0.925\textwidth}
                \vspace{0.25cm}
                \begin{itemize}
                    \item TinyML based system gave a \alert{promising performance} even though the Cloud based
                        system was better in terms of \alert{accuracy}.
                    \item On an \alert{economic bases}, TinyML based system was \alert{more feasible} than the Cloud based one.
                \end{itemize}
                \vspace{0.25cm}
            \end{minipage}
        }
...
%% Case Study: Micro Climate Management
\begin{frame} {}
    ...
    \begin{tikzpicture} [
        ...
        \tikzframetitleright{Case Study:} {Micro Climate Management}
        ...
        \putCsPicInSquare {} {} {csPic} {height = \csPicSquarePadSide,} {pics/microclimate.png}
        ...
        node [anchor = north west] {
            \begin{minipage} {9.85cm}
                \begin{itemize}
                    \item Focused on developing a TinyML solution for an \alert{autonomous strawberry greenhouse} microclimate control system based
                        on multi-variate sensed data.
                    \item Used an \alert{MLP} architecture and experimented with several hyperparameters.
                \end{itemize}
            \end{minipage}
        }
        ;
        ...
        node [
            ...
            \begin{minipage} {0.925\textwidth}
                \vspace{0.25cm}
                \begin{itemize}
                    \item Trained several model instances (90), and one of the model scored \alert{97\%} in validation
                        sets and \alert{96\%} on test sets.
                    \item The developed model was a \alert{light weight} one, thanks to the TinyML approach employed.
                \end{itemize}
                \vspace{0.25cm}
            \end{minipage}
        }
...
\begin{frame} {}
    ...
    \begin{tikzpicture} [
        ...
        \tikzframetitleleft{Case Study:} {Fruit Fly Trap}

        \draw
        (current page.north east) ++(-1, -1) coordinate (csPic)
        ;

        \subCsPicSquare {csPic} {csPic} {topRightCorner}

        \putCsPicInSquare {} {} {csPic} {width = \csPicSquareSide,} {pics/trap.png}
        ...
        (csPic-botLeftCorner |- csPic-pBot) ++(0, -0.1cm) coordinate (lineStart)
        (lineStart -| current page.west) ++(1, 0) coordinate (lineStart)
        (lineStart -| current page.east) ++(-1, 0) coordinate (lineEnd)
        ...
        \draw
        (frametitle.south -| csPic-pLeft) ++(0, -0.3)
        node [anchor = north east] {
            \begin{minipage} {9.85cm}
                \begin{itemize}
                    \item Developed a TinyML based \alert{Fruit fly surveillance system}, implemented using \alert{ESP-EYE}
                        mircrocontroller.
                    \item Used \alert{FOMO} (Faster Objects, More Objects) algorithm developed on Edge Impulse platform.
                \end{itemize}
            \end{minipage}
        }
        ;
        ...
        node [
            ...
            \begin{minipage} {0.925\textwidth}
                \vspace{0.25cm}
                \begin{itemize}
                    \item The system performed with a maximum RAM usage of \alert{2.4Mb} and an inference time
                        of \alert{5.694} seconds.
                    \item The FOMO model only required \alert{53 kilo bytes} of flash memory.
                \end{itemize}
                \vspace{0.25cm}
            \end{minipage}
        }
...
\begin{frame} {}
    ...
    \begin{tikzpicture} [
        ...
        \tikzframetitleleft{Conclusion:} {Key Take Aways}
        ...
        \begin{scope}
            ...
            ;
            \draw
            (current page.south east) ++(0, -4.25)
            node [anchor = south east, inner sep = 0pt]  {
                \includegraphics[width = 0.5\paperwidth]{pics/field.png}
            }
            ;
        \end{scope}
        ...
        \node [anchor = north west]  (content) at (content) {
            \begin{minipage} {0.45\textwidth}
                ...
                \begin{itemize}
                    \item \alert{TinyML} provides a way to bring ML to low power edge devices.
                    \item This enables ML to be used in a plethora of \alert{Embedded Applications}.
                    \item Paves a way for the adoption of \alert{Edge Computing}.
                    \item \alert{Agricultural Sector} could greatly benefit from TinyML,
                        as it encourages to use ML in \alert{Remote} and \alert{Resource Constrained} systems
                        in an \alert{Economically Viable} manner.
                \end{itemize}
...
\begin{frame}[allowframebreaks] {}
    ...
    \nocite{ieee01africa, microclimate, fruitfly, ieee03, sd00, quantization, ieee05, sd02}
...
\begin{frame} {}
    \begin{tikzpicture} [
        ...
        \draw (0, 0)
        coordinate (origin)
        (current page.west) ++(-10.0, 6.5) coordinate (edge)
        ;
        ...
        \begin{scope}
            ...
            \begin{scope}
                \node [anchor = west, inner sep = 0pt] (img) at (current page.west) {
                    \includegraphics[height = \paperheight]{pics/red_forest.png}
                }
                ;
        ...
        \draw (current page.center) coordinate (frametitle) ;
        \node [anchor = center] (frametitle) at (frametitle) {
            \begin{minipage} {9cm}
                \centering
                \bfseries
                \setstretch{0.95}
